...
#         # Load related data
#         course.learning_objectives = db.query(LearningObjective).filter(
#             LearningObjective.course_id == course_id
#         ).all()
        
#         course.target_audiences = db.query(TargetAudience).filter(
#             TargetAudience.course_id == course_id
#         ).all()
        
#         course.requirements = db.query(Requirement).filter(
#             Requirement.course_id == course_id
#         ).all()
        
#         course.sections = db.query(Section).filter(
#             Section.course_id == course_id
#         ).all()
...
#         if status == "disabled":
#             course.is_published = False
#         elif status == "published" and course.verification_status == VerificationStatus.VERIFIED.value:
#             course.is_published = True
...
class CourseService:
    ...
    def get_course_by_id(db: Session, course_id: int):
        ...
        for section in course.sections:
            section.lessons = db.query(Lesson).filter(
