...
from aetherium.models.courses import Topic
...
@router.post("/update_categories", response_model=CategoryResponse)
async def create_category(
    ...
    if not current_user.role.name == "admin":
        raise HTTPException(status_code=403, detail="Not authorized")
    return CourseService.update_category(db, category_data)
