...
const handleAssessmentChange = useCallback((assessment) => {
  ...
  useEffect(() => {
    ...
    // Handle assessment type lessons
    if (lesson.content_type === "ASSESSMENT") {
      setLessonData({
        ...lesson,
        assessment: lesson.assessments?.[0] || {
      ...
      lessonData.assessment.questions.forEach((question, index) => {
        ...
        if (question.options?.length > 0) {
          ...
          if (question.options.length < 2) {
            newErrors[`question_${index}_options`] = `Question ${index + 1} needs at least two options`;
          }

          // Must have a correct answer selected
          if (!question.correct_answer) {
            newErrors[`question_${index}_answer`] = `Question ${index + 1} needs a correct answer selected`;
          }
        } else if (typeof question.correct_answer !== 'string' || !question.correct_answer.trim()) {
