...
//   toggleSection,
// }) => {
// // console.log(section,'sectionnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn')
...
const SectionManager = ({ sections, onChange, onFileUpload, loading }) => {
  const [expandedSections, setExpandedSections] = useState([0])
  const [editingLesson, setEditingLesson] = useState(null)
  const [isLessonEditorOpen, setIsLessonEditorOpen] = useState(false)
  ...
  const saveLesson = async (lessonData) => {
    try {
      ...
      if (!lessonData.name || !lessonData.content_type) {
        alert("Please fill in all required fields (name and content type)")
        return
      }
      ...
      if (lessonData.content?.file) {
        try {
          const uploadedUrl = await onFileUpload(lessonData.content.file)
          lessonData.content_url = uploadedUrl
          delete lessonData.content.file
        } catch (error) {
